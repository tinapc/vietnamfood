...
class Template
{
	 ...
	 * @return	void
	 */
	public function add_js($line, $module_name = NULL)
	{
		$obj =& get_instance();
		...
		{
			$asset_location = $base_url . 'modules/'.$module_name.'/js/';
		} else {
			$asset_location = $base_url . 'themes/'. $this->_theme . '/js/';
		}
		
		$this->_metadata[] = '<script type="text/javascript" src="' . $asset_location . $line . '"></script>';
		return $this;
	}
	 ...
	 * @return	void
	 */
	public function set_metadata($name, $content, $type = 'meta', $module_name = NULL)
	{
		$obj =& get_instance();
		$base_url = $obj->config->item('base_url');
		...
		if(!empty($module_name))
		{
			$asset_location = $base_url . 'modules/'.$module_name.'/';
		} else {
			$asset_location = $base_url . 'themes/'. $this->_theme .'/css/';
		}
		...
		switch($type)
		{
			...
			case 'link':
				$this->_metadata[$content] = '<link rel="'.$name.'" href="' . $asset_location . $content.'" type="text/css" />';
			break;
		}
